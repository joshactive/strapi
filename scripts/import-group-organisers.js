...
// Helper to convert field names from CSV to camelCase
function toCamelCase(str) {
  return str
    .replace(/[-_](\d+)/g, '$1') // keep numbers
    .replace(/[-_](.)/g, (_, c) => c.toUpperCase())
    .replace(/^(.)/, (_, c) => c.toLowerCase())
    .replace(/039s/g, ''); // remove encoded apostrophes
}
      ...
      .on('end', async () => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          try {
            ...
            // Create entry via Strapi API
            const API_TOKEN = process.env.STRAPI_API_TOKEN;
            if (!API_TOKEN) {
              throw new Error('STRAPI_API_TOKEN environment variable is required');
            }
            
            const response = await fetch('http://localhost:1337/api/group-organisers', {
              method: 'POST',
              headers: {
            ...
            if (response.ok) {
              ...
              errorCount++;
              console.error(`❌ [${i + 1}] Failed to import "${data.title}": ${error}`);
            }
            
          } catch (error) {
            errorCount++;
            console.error(`❌ [${i + 1}] Error processing row: ${error.message}`);
          }
