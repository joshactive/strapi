...
const CSV_FILE_PATH = '/Users/joshuathompson/Desktop/content-types/junior-tennis-camps-export.csv';
const STRAPI_API_URL = 'http://localhost:1337/api/junior-tennis-camps';
...
function mapCsvToStrapi(row) {
  ...
  const strapiData = {
    ...
    tennisStandard: row['tennis-standard-junior-camps'] || '',
    ...
    boardBasis: row['board-basis-lg'] || row['board_basis_masterv2'] || '',
...
async function importJuniorCamps() {
  console.log('🚀 Starting Junior Tennis Camps Import...\n');
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(CSV_FILE_PATH)
      ...
      .on('end', async () => {
        ...
        console.log(`✅ Parsed ${camps.length} junior camps from CSV\n`);
        ...
        try {
          ...
          if (testResponse.status === 403) {
            ...
            console.log('   3. Scroll to "Junior Tennis Camp" permissions');
        ...
        // Import each camp
        console.log('📤 Importing junior camps into Strapi...\n');
        ...
        if (successCount > 0) {
          ...
          console.log('   Settings → Roles → Public → Junior Tennis Camp → Uncheck "create"\n');
        }
...
// Run the import
importJuniorCamps().catch(console.error);
