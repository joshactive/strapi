...
const CSV_FILE_PATH = '/Users/joshuathompson/Desktop/content-types/padel-tennis-holidays-export.csv';
const STRAPI_API_URL = 'http://localhost:1337/api/padel-tennis-holidays';
...
function mapCsvToStrapi(row) {
  ...
  const strapiData = {
    ...
    padelStandard: truncateString(row['padel-tennis-standard'] || '', 255),
    ...
    padelCourtSurface: truncateString(row['tennis_court_surface_master'] || row['court-surface-tf'] || '', 255),
    numberOfPadelCourts: truncateString(row['number-of-padel-court'] || row['number-of-courts-tf'] || '', 255),
    distanceFromAirport: truncateString(row['distance-from-airport-pt'] || row['distance-from-airport-tf'] || '', 255),
    padelCourtsInfo: row['padel_courts_info'] || row['tennis_courts_master_v2'] || '',
...
async function importPadelHolidays() {
  console.log('🚀 Starting Padel Tennis Holidays Import...\n');
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(CSV_FILE_PATH)
      ...
      .on('end', async () => {
        ...
        console.log(`✅ Parsed ${holidays.length} padel holidays from CSV\n`);
        ...
        try {
          ...
          if (testResponse.status === 403) {
            ...
            console.log('   3. Scroll to "Padel Tennis Holiday" permissions');
        ...
        // Import each holiday
        console.log('📤 Importing padel holidays into Strapi...\n');
        ...
        if (successCount > 0) {
          ...
          console.log('   Settings → Roles → Public → Padel Tennis Holiday → Uncheck "create"\n');
        }
...
// Run the import
importPadelHolidays().catch(console.error);
