...
const CSV_FILE_PATH = '/Users/joshuathompson/Desktop/content-types/ski-holidays-export.csv';
const STRAPI_API_URL = 'http://localhost:1337/api/ski-holidays';
...
function mapCsvToStrapi(row) {
  ...
  // Parse itinerary
  const itinerary = parseItineraryItems(row['new-itinerary-ski'] || row['itinerary_master'] || '');

  // Parse FAQs
  const faqs = parseFaqItems(row['faq_master'] || '');

  // Parse inclusions (what's included)
  const inclusionsData = parseInclusionItems(row['ski-what039s-included'] || row['what039s_included-master'] || '', true);
  const exclusionsData = parseInclusionItems(row['skiwhat039s-not-included'] || row['what039s_not_included-master'] || '', false);
  const inclusions = [...inclusionsData, ...exclusionsData];

  // Parse facilities
  const facilities = parseInclusionItems(row['ski-included-facilities'] || row['included-facilities'] || row['facilities_master_v2'] || '', true);

  // Extract slug
  const slug = extractSlug(row['Permalink']) || row['unique_value_master'] || '';

  // Parse price
  const priceFrom = parseFloat(row['ski-price'] || row['price-from-lg'] || 0);
  ...
  const strapiData = {
    ...
    lengthOfTrip: truncateString(row['ski-length-of-trip'] || row['length_of_trip'] || '', 255),
    ...
    mainInformation: row['main-information'] || row['main-information-tf'] || '',
    skiHolidayType: truncateString(row['ski-holiday-type'] || '', 255),
    pistes: truncateString(row['pistes'] || '', 255),
    levelOfSkiing: truncateString(row['level-of-skiing'] || '', 255),
    perfectFor: truncateString(row['ski-perfect-for'] || '', 255),
    singleOccupancy: row['ski-single-occupancy'] || row['single_occupancy_range_master'] || '',
    singleOccupancyShort: row['ski-single-occupancy_short'] || '',
    priceFrom: isNaN(priceFrom) ? null : priceFrom,
    boardBasis: truncateString(row['ski-board-basis'] || row['board_basis_masterv2'] || row['board-basis-lg'] || '', 255),
    starRating: truncateString(row['ski-star-rating'] || row['star-rating-tf'] || '', 255),
    departureMonth: truncateString(row['ski-departure-month'] || '', 255),
    arrivalDepartureDay: truncateString(row['ski-arrival-amp-departure-day'] || '', 255),
    ...
    distanceFromAirport: truncateString(row['ski-distance-from-airport'] || row['distance-from-airport-tf'] || '', 255),
    averageGroupSize: truncateString(row['ski-average-group-size'] || '', 255),
    settingDescription: row['setting_master_v2'] || '',
    boardBasisIncluded: row['ski-what-does-my-board-basis-include'] || '',
    restaurantInformation: row['skirestaurant-information'] || row['restaurants_v2_master'] || '',
    barInformation: row['ski-bar-information'] || row['bars_master_v2'] || '',
    roomsInformation: row['ski-rooms'] || row['rooms-tf'] || '',
    topTips: row['top_tips_master_v2'] || '',
    moreInformation: row['more-information-padel'] || '',
    gettingThere: row['getting_there'] || '',
    googleMapsUrl: row['google-maps-ski'] || row['google-maps-ta'] || '',
    locationAddress: row['ski-location-address'] || row['ski-location-for-page'] || '',
    itineraryDownloadUrl: row['ski-download-itinerary'] || row['itinerary_download_url'] || '',
    otherFaqsUrl: row['other_faqs_url_master'] || '',
    typicalGroupSize: truncateString(row['typical_group_size_master'] || '', 255),
...
async function importSkiHolidays() {
  console.log('🚀 Starting Ski Holidays Import...\n');
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(CSV_FILE_PATH)
      ...
      .on('end', async () => {
        ...
        console.log(`✅ Parsed ${holidays.length} ski holidays from CSV\n`);
        ...
        try {
          ...
          if (testResponse.status === 403) {
            ...
            console.log('   3. Scroll to "Ski Holiday" permissions');
        ...
        // Import each holiday
        console.log('📤 Importing ski holidays into Strapi...\n');
        ...
        if (successCount > 0) {
          ...
          console.log('   Settings → Roles → Public → Ski Holiday → Uncheck "create"\n');
        }
...
// Run the import
importSkiHolidays().catch(console.error);
